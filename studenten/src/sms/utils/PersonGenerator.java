...
public class PersonGenerator {
    ...
    // we maken een willekeurige persoon
    public static Person createRandomPerson() {
        ...
        int telefoonnummer = new Random().nextInt(29999999) + 470000000;
        int heeftTelefoon = new Random().nextInt(10);

        String naam = voornamen[voornaam] + " " + familienamen[familienaam];

        if (heeftTelefoon % 2 == 0) {
            person = new Person(naam, straten[straat], postcodes[postcode], gemeenten[postcode]);
        }
        else {
            person = new Person(naam, straten[straat], postcodes[postcode], gemeenten[postcode], "0" + String.valueOf(telefoonnummer).replaceFirst("(\\d{3})(\\d{3})(\\d+)", "$1/$2.$3"));
        }
    ...
    public static ArrayList<Person> createRandomListOfPersons() {

        int aantal = new Random().nextInt(50) + 25;
        persons = new ArrayList<>();

        VaccinLogger.log().debug("Starten met het aanmaken van een willekeurige set van personen. " + aantal + " in dit geval.");
        for (int i = 0; i < aantal; i++) {
            persons.add(createRandomPerson());
        }
        VaccinLogger.log().debug("Stop, willekeurige set van personen werd aangemaakt.");
