...
public class TestController {
	...
	public void startWatchDog(JPanel panel, JLabel remaining) {
		...
		TimeLimitThreadImpl watchDog = new TimeLimitThreadImpl(getStopTime(), remaining, score);
		Thread thread = new Thread(watchDog);
		thread.setName("DEDS Timer");
		thread.start();
	}
	public java.util.Date getStopTime() {
		return new Date(System.currentTimeMillis()+ test.getLimitSeconds()* Constants.SECONDMILLIS);
	}
}
