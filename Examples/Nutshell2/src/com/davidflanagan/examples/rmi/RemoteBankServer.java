...
public class RemoteBankServer extends UnicastRemoteObject implements RemoteBank
{
	 ...
	 **/
	public synchronized void openAccount(final String name, final String password) throws RemoteException, BankingException
	{
		...
		final Account acct = new Account(password);
		// And register it
	 ...
	 **/
	Account verify(final String name, final String password) throws BankingException
	{
		synchronized (accounts)
		{
			final Account acct = (Account) accounts.get(name);
			if (acct == null)
	 ...
	 **/
	public synchronized FunnyMoney closeAccount(final String name, final String password) throws RemoteException, BankingException
	{
		Account acct;
		...
		synchronized (acct)
		{
			final int balance = acct.balance;
			acct.balance = 0;
	...
	public void deposit(final String name, final String password, final FunnyMoney money) throws RemoteException, BankingException
	{
		final Account acct = verify(name, password);
		synchronized (acct)
		{
	...
	public FunnyMoney withdraw(final String name, final String password, final int amount) throws RemoteException, BankingException
	{
		final Account acct = verify(name, password);
		synchronized (acct)
		{
	...
	public int getBalance(final String name, final String password) throws RemoteException, BankingException
	{
		final Account acct = verify(name, password);
		synchronized (acct)
		{
	 ...
	 **/
	public List getTransactionHistory(final String name, final String password) throws RemoteException, BankingException
	{
		final Account acct = verify(name, password);
		synchronized (acct)
		{
	...
	public static void main(final String[] args)
	{
		try
		{
			...
			final RemoteBankServer bank = new RemoteBankServer();
			// Figure out what to name it
			final String name = System.getProperty("bankname", "FirstRemote");
			// Name it that
