...
public class WWLog extends JFrame implements ActionListener
{
    ...
    private void loadListBox()
    {
        ...
        try
        {
           ...
           BufferedReader newFileStream =
              new BufferedReader(new InputStreamReader( new FileInputStream(logFileName)));
           boolean done = false;
           while (!done)
           {
              String line = newFileStream.??(); // read one line
              if (line!=null)
                {
    ...
    private boolean copyToFile(String newFile)
    {
        ...
        try
        {
           ...
           PrintWriter logFileStream =
             new ?? ; // instantiate printwriter object

           // loop through input file, writing to output file
           boolean done = false;
           while (!done)
           {
              String line = inFileStream.??(); // read input line
              if (line != null)
                logFileStream.??(line); // write output line to the log file
              else
                done = true;
           } // end while-loop
           logFileStream.flush();
           inFileStream.??();  // close the input file
           logFileStream.??(); // close the output file
