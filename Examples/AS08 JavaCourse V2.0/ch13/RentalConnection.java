...
/**
 * A class for maintaining a connection to the car rental database. Also includes prepared statements
 * needed by the rent and return actions, for better performance. Usage: instantiate, passing the name/tcp-address of the
 * system to connect to, as well as the library and file name of the car db. connect(), which returns true/false if it worked.
 * Pass the userId and password, or leave null to be prompted. If false returned, call getLastSQLError()
 * prepare(), to prepare often needed statements. If false returned, call getLastError()
 * getxxx(), when you need to get the system name, library name, file name, or prepared statement objects
...
public class RentalConnection {
     ...
     */
    public RentalConnection(String system, String library, String file) {
    ...
    public boolean prepare() {
        ...
        boolean ok = false;
        try {
            String sql = "UPDATE " + libfile + " SET STATUS = 'N'" + ", RETDATE = ? WHERE PLATE = ?";
